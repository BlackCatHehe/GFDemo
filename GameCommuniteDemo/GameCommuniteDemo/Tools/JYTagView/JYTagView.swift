...
public class JYTagView: UIView {
    ...
    /// item的width,默认为nil,若设置，则item宽度固定
    ...
    /// 左上角的一个功能性button,默认不存在（一般用来设置点击折叠）
    public var moreButton: UIButton?
    
    /// moreButton和tagview的间距
    public var buttonMargin: CGFloat = 40.0
...
public extension JYTagView {
    ...
    public func reloadData() {
        ...
        for i in 0..<titles.count {
            ...
            let bWidth = itemWidth == nil ? title.jy_getSizeWith(font: titleFont, maxSize: CGSize(width: self.frame.width, height: itemHeight)).width + 2*itemInsetPadding : itemWidth!
            
            
            //如果有moreButton,则第一行的宽度需要加上moreBt来判断
            if let moreBt = moreButton, row == 0 {
                if (tagX + bWidth + moreBt.frame.width + buttonMargin) > self.frame.width {
                    row += 1
                    tagX = 0
