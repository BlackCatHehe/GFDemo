    ...
    private func initCollectionView() {
        ...
        collectionView.mj_footer =  MJRefreshBackNormalFooter(refreshingBlock: {
            ...
            if let cid = self.cateModels[self.selectIndex].id{
                self.requestGoodsList(with: cid, page: self.currentPages[self.selectIndex] + 1)
            }
        })
    }
}
...
extension GCMyItemsVC: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
...
extension GCMyItemsVC {
    ...
    @objc private func selectTag(_ sender: UIButton) {
        ...
        if let cid = cateModels[selectIndex].id{
            ...
            self.requestGoodsList(with: cid, page: currentPages[selectIndex])
        }
    }
...
extension GCMyItemsVC {
        ...
        GCNetTool.requestData(target: GCNetApi.goodsCate, controller: self, showAcvitity: true, success: { (result) in
            if let data = result["data"] as? [[String: Any]] {
                ...
                if let cid = self.cateModels.first?.id{
                    ...
                    self.requestGoodsList(with: cid, page: 1)
                }
    ...
    private func requestGoodsList(with cid: Int, page: Int) {
        
        guard page >= 1 else {
            JYLog("页数不能小于1")
            return
        }
        let prama = ["category_id" : cid]
        
        GCNetTool.requestData(target: GCNetApi.myItems(prama: prama), controller: self, showAcvitity: false, isTapAble: false, success: { (result) in
