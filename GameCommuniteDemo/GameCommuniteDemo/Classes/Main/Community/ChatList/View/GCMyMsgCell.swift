...
class GCMyMsgCell: UITableViewCell, Reusable {
    ...
    func setModel(isHaveTime: Bool = false, message: NIMMessage) {
        ...
        if let user = GCUserDefault.shareInstance.userInfo, let avatar = user.avatar {
            iconImgV.kfSetImage(url: avatar, targetSize: CGSize(width: adaptW(44.0), height: adaptW(44.0)), cornerRadius: adaptW(22.0))
        }
...
extension GCMyMsgCell {
    ...
    private func setupUI(){
        ...
        contentImgV.image = UIImage(named: "chat_bg_self")
        ...
        contentBgV.snp.makeConstraints { (make) in
            make.right.equalTo(iconImgV.snp.left).offset(-adaptW(5.0))
        ...
        contentImgV.snp.makeConstraints { (make) in
            ...
            make.left.equalTo(contentLb).offset(-10.0)
            make.right.equalTo(contentLb).offset(15.0)
