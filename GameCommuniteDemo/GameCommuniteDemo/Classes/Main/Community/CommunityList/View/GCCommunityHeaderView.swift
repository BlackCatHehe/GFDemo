...
extension GCCommunityHeaderView {
    ...
    private func initUI() {
        ...
        let tapMember = UITapGestureRecognizer(target: self, action: #selector(tapEnterMemberList))
        memberV.addGestureRecognizer(tapMember)
    ...
    @objc private func tapEnterMemberList() {
        delegate?.headerViewDidTapEnterMemberList(self)
    }
}
