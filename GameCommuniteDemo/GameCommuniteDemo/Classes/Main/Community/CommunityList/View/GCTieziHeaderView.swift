...
extension GCTieziHeaderView {
    ...
    private func initUI() {
        ...
        let tapContent = UITapGestureRecognizer(target: self, action: #selector(tapMain))
        addGestureRecognizer(tapContent)
        ...
        let tapIcon = UITapGestureRecognizer(target: self, action: #selector(tapIconImg))
        iconImgV.addGestureRecognizer(tapIcon)
    ...
    @objc func clickButtonButtons(_ sender: UIButton) {
        ...
        delegate?.headerView(self, didSelectItemAt: sender.tag - 100, sender: sender)
    ...
    @objc func tapMain() {
        ...
        delegate?.headerViewDidBeSelected(self)
        
    }
    
    @objc func tapIconImg() {
        delegate?.headerViewDidTapUserIcon(self)
    }
}
