...
class GCArticalBottomView: UIView {
    ...
    func setModel(_ model: GCTopicModel) {
        contentTV.placeholder = "评论(已有\(model.commentCount ?? 0)条评论)"
        
        commentBt.setTitle("\(model.commentCount ?? 0)", for: .normal)
        commentBt.layoutButton(style: .Left, imageTitleSpace: 5.0)
        
        likeBt.setTitle("\(model.likeCount ?? 0)", for: .normal)
...
extension GCArticalBottomView: UITextViewDelegate {
    func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange, replacementText text: String) -> Bool {
        if text == "\n" {
            ...
            delegate?.bViewDidClickPost(self, text: contentTV.text)
            return false
        }
        return true
